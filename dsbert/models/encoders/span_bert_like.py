...
class SpanBertLikeConfig:
    def __init__(self, **kwargs):
        ...
        self.share_weights_ext = kwargs.pop('share_weights_ext', True)
        # Share weights internally, i.e., between `query_bert_like` of different span sizes
        self.share_weights_int = kwargs.pop('share_weights_int', True)
...
class SpanBertLikeEncoder(torch.nn.Module):
    def __init__(self, config: SpanBertLikeConfig):
        ...
        if config.share_weights_int:
            ...
            self.query_bert_like = QueryBertLikeEncoder(
                config.bert_like.encoder, num_layers=config.num_layers, share_weights=config.share_weights_ext)
        else:
    ...
    def forward(self, all_hidden_states: List[torch.Tensor]):
        ...
        for k in range(2, min(self.max_span_size, num_steps)+1):
            ...
            reshaped_states = [hidden_states.unfold(dimension=1, size=k, step=1).permute(0, 1, 3, 2).flatten(end_dim=1)for hidden_states in all_hidden_states]
            ...
            if self.share_weights_int:
                ...
                # `k` starts from 2
                query_outs = self.query_bert_like[k - 2](query_states, reshaped_states)
