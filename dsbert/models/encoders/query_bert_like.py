...
class QueryBertLikeAttention(torch.nn.Module):
    def __init__(self, origin: transformers.models.bert.modeling_bert.BertAttention, share_weights: bool = False):
        super().__init__()
        self.self = QueryBertLikeSelfAttention(
            origin.self, share_weights=share_weights)
        if share_weights:
...
class QueryBertLikeLayer(torch.nn.Module):
    ...
    def forward(self, query_states, hidden_states, **kwargs):
        self_attention_outputs = self.attention(
            query_states, hidden_states, **kwargs)
...
class QueryBertLikeEncoder(torch.nn.Module):
    ...
    def __init__(self, origin: transformers.models.bert.modeling_bert.BertEncoder, num_layers: int = None, share_weights: bool = False):
        ...
        self.layer = torch.nn.ModuleList([QueryBertLikeLayer(
            layer, share_weights=share_weights) for layer in origin.layer[-num_layers:]])

    def forward(self,
                query_states,  # [batch_size * num_spans, 1, hidden_size]
                all_hidden_states,  # List[[batch_size * num_spans, span_size, hidden_size]], len = num_layers + 1
                attention_mask=None,
                head_mask=None,
